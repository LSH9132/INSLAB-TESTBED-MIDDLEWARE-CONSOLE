  ...
  return (
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-gray-800">
        ...
        <span className="text-xs text-gray-400">원격 수집 (5s)</span>
      </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              <div className="flex items-center justify-between">
                <span className="text-xs text-gray-500 dark:text-gray-400"><span className="text-green-500">↓</span> 수신 (RX)</span>
                <span className="text-sm font-mono font-semibold text-green-600 dark:text-green-400">{formatBps(currentIface.rxBps)}</span>
              </div>
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between">
                <span className="text-xs text-gray-500 dark:text-gray-400"><span className="text-blue-500">↑</span> 송신 (TX)</span>
                <span className="text-sm font-mono font-semibold text-blue-600 dark:text-blue-400">{formatBps(currentIface.txBps)}</span>
              </div>
              ...
              <tbody>
                {interfaces.map((iface) => (
                  <tr
                    key={iface.iface}
                    onClick={() => setSelectedIface(iface.iface)}
                    className={`cursor-pointer border-b border-gray-50 dark:border-gray-800/50 ${
                      iface.iface === active ? 'bg-blue-50 dark:bg-blue-900/20' : 'hover:bg-gray-50 dark:hover:bg-gray-800/50'
                    }`}
                  >
